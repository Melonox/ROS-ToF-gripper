...
class Servo:
    ...
    def __init__(self, numServos, mode="nervous", target_distance=2):
        ...
        self.servo_pins = np.array([4, 5, 6, 7, 15]) #should have same length as numservos
        ...
        self.previous_error = [0]*self.numServos
        self.previous_time = time.time()
        rospy.Subscriber('/servo/command', UInt16MultiArray, self.servo_callback, queue_size=1, buff_size=2**24)
        self.pub_position = rospy.Publisher('/servo/position', Float32MultiArray, queue_size=1)
        self.activate_fingers = False
        self.servo_command = np.zeros(self.numServos)
    ...
    def set_angles(self, arrOfAngles):
        for i in range(self.numServos):
            print(f'Set servo {i} to {arrOfAngles[i]}')
            self.myKit.servo[self.servo_pins[i]].angle = arrOfAngles[i]
    
    # ROS callback
    def servo_callback(self, msg):
        self.servo_command = np.array(msg.data)
        if self.activate_fingers:
...
if __name__ == '__main__':
    ...
    num_servos = rospy.get_param('num_servos', 2)
    ...
    # Reset Servos
    servo.set_angles([0,0,0,0])
    rospy.sleep(5)
    
    # Counter for datagathering
    dataseset_counter = 0

    servo.activate_fingers = True
    while not rospy.is_shutdown():
        print(servo.servo_command)
        if np.all(servo.servo_command[:servo.numServos] < 35):
            servo.activate_fingers = False
            servo.set_angles([0,0,0,0])
            rospy.sleep(3)

            dataseset_counter = dataseset_counter + 1
            if dataseset_counter % 2 == 0:
                angle = servo.myKit.servo[15].angle
                print('Adjusting target to {angle + 5}')
                servo.myKit.servo[15].angle = angle + 5
